...
class Expectimax:
    ...
    def maximizer_node(self, board: np.ndarray, depth: int):
        ...
        moves = gf.get_moves()
        for move in moves:
            if move == gf.move_right:
                if 0 not in board[3] and 0 not in board[2]:
                    new_board, move_made, _ = move(copy_board)
                    if move_made:
                        score, _ = self.expectimax(new_board, depth - 1, 0)
                        if score > best_score:
                            best_score = score
                            best_action = move
            elif move == gf.move_up:
                if 0 not in [board[0][0], board[1][0], board[2][0], board[3][0], board[0][1], board[1][1], board[2][1], board[3][1]]:
                    new_board, move_made, _ = move(copy_board)
                    if move_made:
                        score, _ = self.expectimax(new_board, depth - 1, 0)
                        if score > best_score:
        ...
        if best_action is None:
            new_board, move_made, _ = gf.move_up(copy_board)
            if move_made:
                score, _ = self.expectimax(new_board, depth - 1, 0)
                if score > best_score:
                    best_score = score
                    best_action = gf.move_up
            new_board, move_made, _ = gf.move_right(board)
            if move_made:
                score = self.expectimax(new_board, depth - 1, 0)[0]
                if score > best_score:
                    best_score = score
                    best_action = gf.move_right
    ...
    def chance_node(self, board: np.ndarray, depth: int):
        ...
        for row, col in zip(empty_cells[0], empty_cells[1]):
            ...
            tile_value = 2 if random_value < 0.9 else 4  # 90% for 2, 10% for 4
